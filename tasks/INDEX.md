...
## Фаза 8 — GUI «Индивидуально»: выбор дней реально управляет записью [DONE]
- `phase_8_01_individual_day_selection.md` — per-day `ConfirmDialog` убран; синхронный перехват
  `builtins.input` в `FillWorker` пишет только отмеченные в панели дни своими инлайн-значениями;
  блокировка недоступных дней (`classify_day_states`/`DayStatesWorker`, read-only) + кнопка
  «↻ Обновить». `src/` не изменён; метод «По-умолчанию» и export не затронуты.
- Отчёт: `../tasks_done/phase_8_done.md` (2026-07-01). 369 passed. Известные ограничения:
  «ремонт» закрытия дел идёт по уже заполненным дням окна независимо от галок; отмеченный день вне
  топ-5 `run_fill` не запишется (страховка `_reread_guard`); возможен полночный сдвиг дат панели
  vs статусов. Визуальная сверка собранного окна с макетом и живой REST-прогон — НЕ проводились
  (документационная задача), за пользователем.
