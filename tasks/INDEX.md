...
- **Доработка (2026-07-01, тот же отчёт, отдельная секция):** по требованию заказчика dry-run
  ...
  `ConfirmDialog` дополнительно для «Индивидуально»); дефолты «Описание/Часы» редактируются
  инлайн в колонке «Редактирование» (отдельный диалог «Значения по-умолчанию» убран как дубль).
  CLI dry-run не затронут.
